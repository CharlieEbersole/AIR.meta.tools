...
format_keywords = function(data, exclude = NULL, ...) {
  data %>% 
    filter(include == 1, !(keyword %in% exclude)) %>%
    plyr::dlply("group", pull, "keyword") %>%
    lapply(write_group, ...)
}

#helper function to write the string for one group of keywords
write_group = function(words, quotes = TRUE, quotes_all = FALSE,
                       brackets = FALSE) {
  
  #first remove extra spaces from the keywords if needed
  words = str_squish(words)
  
  #add quotes to everything if applicable
  if (quotes_all) {
    words = paste0('"', words,'"')
  } 
  
  #add quotes to multi-word phrases if applicable
  if (!quotes_all & quotes) {
    multi_word = str_detect(words, " |-")
    words = ifelse(multi_word,
                   paste0('"', words,'"'),
                   words)
  }
  
  #replaces quotes with brackets - used for Scopus
  ...
  #other formatting
  words = words %>% str_replace_all('""', '"')
  glue::glue("{paste0(words, collapse = ' OR ')}")
}

#allows to search for specific fields (e.g., title, abstract, keywords)
specific_fields = function(search_string, fields = c("TI", "AB", "KW"), 
                           scopus = FALSE, ignore_if = NULL) {
  
  #use broadest possible fields if search_string contains ignore_if...
  if (!is.null(ignore_if)) {
...
#user-facing function to get a search string from a database of keywords
search_db = function(keywords, db = "ebsco", fields = NULL) {
  
  #db specific customizations
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "wos") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "wos") == TRUE, 1)
  
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "ebsco") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "ebsco") == TRUE, 1)
  
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "asp") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "asp") == TRUE, 1)
  
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "psyc") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "psyc") == TRUE, 1)
  
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "es") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "es") == TRUE, 1)
  
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "eric") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "eric") == TRUE, 1)
  
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "soc") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "soc") == TRUE, 1)
  
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "scop") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "scop") == TRUE, 1)
  
  keywords$include = replace(keywords$include, str_detect(keywords$db_remove, "proq") == TRUE, 0)
  keywords$include = replace(keywords$include, str_detect(keywords$db_add, "proq") == TRUE, 1)
  
  #convert from data frame of keywords to grouped list of OR'ed strings
  grps = format_keywords(keywords)
  
  #Web of Science - include quotes around everything
  if (db == "wos") {
    res = keywords %>% 
      format_keywords(quotes_all = TRUE) %>%
      combine_field()
  }
  
  #EBSCO-based databases
  if (db %in% c("ebsco", "asp", "psyc", "es", "eric", "soc")) {
    if (is.null(fields)) fields = c("TI", "AB", "KW")
    res = grps %>% 
      lapply(specific_fields, fields) %>% 
      combine_field()
  }
  
  #Scopus - include brackets around everything
  if (db == "scop") {
    if (is.null(fields)) fields = c("TITLE-ABS", "AUTHKEY")
    res = keywords %>% 
      format_keywords(brackets = TRUE) %>%
      lapply(specific_fields, fields = fields, scopus = TRUE) %>% 
      combine_field()
  }
  ...
  if (db == "proq") {
    res = grps %>% 
      lapply(specific_fields, fields=c("NOFT")) %>% 
      combine_field()
  }
...
#helper function for retrieving the number of search hits from a single EBSCOhost database
get_n_hits_one_db = function(db, 
                             keywords = NULL,
                             query = NULL,
                             ebsco_prof = "s6681610.main.eitws3", 
                             ebsco_pwd = "ebs1540",
                             scopus_api = "7f59af901d2d86f78a1fd60c1bf9426a") {
  
  #print progress and get the search string
  message(paste0("Trying ", db, "..."))
  if (is.null(query)) {
    query = search_db(keywords, db)
    
    #Scopus API complains at super-long search strings, so omit AUTHKEY in this
    #rapid testing phase
    if (db == "scop") query = search_db(keywords, db, "TITLE-ABS")
  }
  

  
  #
  mod = query %>% str_replace_all(" ", "+")
  
  #EBSCOhost databases 
  if (db %in% c("asp", "psyc", "es", "eric", "soc")) {
    
    #crosswalk between the short database names I use and what EBSCO uses
    ebsco_crosswalk = c(
      "eric" = "eric",
      "asp" = "aph",
      "psyc" = "psyh",
      "es" = "eue",
      "soc" = "sih")
    ebsco_db = ebsco_crosswalk[db]
    
    #get the API url and scrape the results
    api_url = paste0("http://eit.ebscohost.com/Services/SearchService.asmx/Search?",
                     "prof=", ebsco_prof, 
                     "&pwd=", ebsco_pwd, 
                     "&query=", mod, 
                     "&db=", ebsco_db)
    n_hits = read_html(api_url) %>% 
      html_node("hits") %>% 
      html_text()
  }
  ...
  #SCOPUS database
  if (db %in% "scop") {
    api_url = paste0("https://api.elsevier.com/content/search/scopus?",
                     "query=", mod, 
                     "&apiKey=", scopus_api) %>% URLencode()
    n_hits = content(GET(api_url))$`search-results`$`opensearch:totalResults`
  }
...
#retrieves the number of search hits from potentially multiple EBSCOhost databases
#INPUTS: query = search string
#        db = short database name (see https://eit.ebscohost.com/Services/SearchService.asmx/Info?prof=s6681610.main.eitws3&pwd=ebs1540&authType=profile&ipprof=)
#        ebsco_prof = EBSCOhost API username
#        ebsco_pwd = EBSCOhost API password
#        scopus_api = Scopus API key
#OUTPUT: number of search hits
get_n_hits = function(keywords,
                      db = c("asp", "psyc", "es", "eric", "soc", "scop"), 
                      ebsco_prof = "s6681610.main.eitws3", 
                      ebsco_pwd = "ebs1540",
                      scopus_api = "7f59af901d2d86f78a1fd60c1bf9426a") {
  db %>% 
    sapply(get_n_hits_one_db,
           keywords = keywords,
           ebsco_prof = ebsco_prof,
           ebsco_pwd = ebsco_pwd,
           scopus_api = scopus_api)
}
...
#OUTPUT: outputs data frame, with a column saying which term is dropped
#        and the search string needed to examine differences between including 
#        vs. excluding each term in multiple databases
change_one = function(keywords, test_rows = NULL) {
  
  #get the base counts
  message("Getting base counts:")
  base_counts = get_n_hits(keywords)
  
  #get base search strings
  ebsco_b = search_db(keywords, "ebsco")
  wos_b = search_db(keywords, "wos")
  scop_b = search_db(keywords, "scop")
  
  #gets the results for omitting a single keyword 
  drop_i = function(i) {
    
    #print progress and skip if the keyword isn't included anyway
    message(paste0("Keyword: ", keywords$keyword[i]))
    if (keywords$include[i] == -1) {
      return(data.frame(ebsco_s = ""))
    }
    
    #modify keywords
    old = keywords$include[i]
    ki = keywords
    ki$include[i] = 1 - old
    
    #get differences in counts
    diffs = tryCatch({ 
      abs(get_n_hits(ki) - base_counts)
    }, error = function(e) NA)
    
    #get search strings with keyword removed
    ebsco_i = search_db(ki, "ebsco")
    wos_i = search_db(ki, "wos")
    scop_i = search_db(ki, "scop")
    
    #get search string for difference
    if (old == 1) {
      ebsco_s = glue("({ebsco_b}) NOT ({ebsco_i})")
      wos_s = glue("({wos_b}) NOT ({wos_i})")
      scop_s = glue("({scop_b}) AND NOT ({scop_i})")
    } else {
      ebsco_s = glue("({ebsco_i}) NOT ({ebsco_b})")
      wos_s = glue("({wos_i}) NOT ({wos_b})")
      scop_s = glue("({scop_i}) AND NOT ({scop_b})")
    }

    #format results as a data frame
    data.frame(data.frame(as.list(diffs)),
               ebsco_s, wos_s, scop_s)
  }
  
  #determine rows to apply the function to
  if (is.null(test_rows)) test_rows = 1:nrow(keywords)
  ...
  #return with original keywords dataframe 
  bind_cols(keywords[test_rows, ], res)
}
...
subtract_one = function(keywords){
  ...
  for(i in 1:length(keywords$keyword)){
    ...
    SCOPUS = glue("({SCOPUSAll}) AND NOT ({SCOPUSDrop1})")
